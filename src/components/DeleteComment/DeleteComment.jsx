...
import { useQueryClient } from '@tanstack/react-query';
  ...
  async function onSubmit(data) {
    console.log("Form Data:", data);
      setIsLoading(true) 
    try {
      let res=await axios.delete(`https://linked-posts.routemisr.com/comments/${id}`,{
        headers:{
  ...
  await query.invalidateQueries({ queryKey: ["home"], exact: true });
      ...
      console.log(error);
       toast.error("not Access to delete comment")
             setIsLoading(false) 
    }
  ...
  return (
    <>
      <Button disabled={IsLoading} className=" my-2 w-[100%] md:w-fit" onClick={() =>onSubmit()}>{IsLoading?<i className="fas fa-spinner fa-spin text-white"></i>:"Delete Comment"}</Button>    </>
  );
}
