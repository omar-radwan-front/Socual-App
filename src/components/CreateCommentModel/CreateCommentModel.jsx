...
export default function CreateCommentModel({ id }) {
  ...
  async function onSubmit(data) {
    ...
    setIsLoading(true)
    try {
      let res=await axios.post(`https://linked-posts.routemisr.com/comments`,data,{
        headers:{
         ...
         toast.success("append comment success")
 await query.invalidateQueries({ queryKey: ["profile"], exact: true });
  await query.invalidateQueries({ queryKey: ["home"], exact: true });
      ...
      console.log(error);
       toast.error("can't add comment error")
       setIsLoading(false)
             setOpenModal(false); // تقفل المودال
  ...
  return (
    <>
      <Button className=" my-2 w-[100%] md:w-fit" onClick={() => setOpenModal(true)}>Add Comment</Button>
